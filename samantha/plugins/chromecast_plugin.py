...
class MediaStatusListener(object):
    ...
    def __init__(self, name):
        """Set up the listener."""
        self.name = "{}_MediaStatusListener".format(name)
    ...
    def new_media_status(self, status):
        ...
        if not status.player_state == self.player_state:
            LOGGER.debug("New state: %s. Previous: %s",
                         status.player_state, self.player_state)
...
class CastStatusListener(object):
    ...
    def __init__(self, name):
        """Set up the listener."""
        self.name = "{}_CastStatusListener".format(name)
...
def onstart(key, data):
    ...
    try:
        ...
        m_listener = MediaStatusListener(PLUGIN.name)
        c_listener = CastStatusListener(PLUGIN.name)
        # Register the listener to the Chromecast's status and media-status
        # events.
        mediacontroller.register_status_listener(m_listener)
        CAST.register_status_listener(c_listener)
    ...
    except pychromecast.ChromecastConnectionError as e:
